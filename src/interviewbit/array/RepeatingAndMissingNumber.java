...
public class RepeatingAndMissingNumber {
    ...
    static void repeatingMissing(int arr[]) {
        ...
        for (int i = 0; i < arr.length; i++) {
            int actual = arr[i] - 1;
            if (arr[i] == newArr[actual]) {
                System.out.println("repeat " + newArr[actual]);
            } else {
                newArr[actual] = arr[i];
            }
        }
        for (int i = 0; i < newArr.length; i++) {
            if (newArr[i] == 0) System.out.println("Missing " + (i + 1));
        }
    }
    static List<Integer> repeatingMissingList(List<Integer> list) {
        ...
        if(list.size()>1 ) {
            for (int i = 0; i < list.size(); i++) {
                int actual = list.get(i) - 1;
                int toMatch = newList.get(actual);
                if(actual==212) {
                    System.out.println(newList.get(actual));
                }
                if (list.get(i) == toMatch) {
                    resList.add(newList.get(actual));
                } else {
                    newList.set(actual, list.get(i));
                }
            }
            for (int i = 0; i < newList.size(); i++) {
                if (newList.get(i) == 0) resList.add(i + 1);
            }return resList;
        }else{
            resList = (ArrayList<Integer>) list;
        }
        return resList;
    }
